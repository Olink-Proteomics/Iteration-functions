...
#'
#' @details
#' The function applies a series of optional filters to the data and categorizes 
#' the numeric values in the specified `count_column` into groups based on 
#' thresholds provided in `value_breaks`. A chi-square test is performed to 
#' compare the distributions of these categories across the levels of the 
#' specified `comparison_column`.
#'
#' The function ensures that:
#' - The filtered data is not empty.
#' - The `count_column` contains numeric values.
#' - The chi-square test operates on a valid contingency table.
#'
#' @examples
...
descriptive_tabulation <- function(data, 
  ...
  if (!requireNamespace("dplyr")) stop("Package 'dplyr' is required.")
  if (!requireNamespace("gtsummary")) stop("Package 'gtsummary' is required.")
  if (!requireNamespace("rlang")) stop("Package 'rlang' is required.")
  ...
  if (!is.null(filter_additional)) {
    data <- data %>% filter(!!parse_expr(filter_additional))
  }
  
  # Check if filtered data is empty
  if (nrow(data) == 0) {
    stop("Filtered data is empty. Check your filtering conditions.")
  }
  
  # Ensure count_column is numeric
  if (!is.numeric(data[[count_column]])) {
    stop("The count_column must contain numeric values.")
  }
  ...
  contingency_table <- table(data$Count_Group, data[[comparison_column]])
