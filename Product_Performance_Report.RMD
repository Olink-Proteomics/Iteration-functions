...
\newpage
\section{Average detectability per block, plasma}
...
```
\newpage
\section{Detectability distribution within block across replicate runs}
...
```{r}
  ...
  #ADD LATER ON FILTER ON CONCORDANCE SAMPLES and remove dependency to assay_group file
assay_group<-yourdata %>% distinct(olink_id, label)
...
calculate_detectability_percentage (results$data, detectability_col= "median_detectability" , assay_group = assay_group, group_var = "label", thresholds = c(0.1,0.5)) %>%
...
process_matrix <- function(matrix_name) {
  ...
  calculate_detectability_percentage(
    ...
    kbl(position = "!h", format = "latex",
        caption = paste0("Percentage of assays detectable in at least one sample in ", matrix_name, " samples"),
        col.names = c("Group", "Percentage")) %>%
    kable_classic(full_width = FALSE)

}

process_matrix("Plasma")
